...
nginx_setup() {
  ...
  if [[ ! -e /etc/nginx/server.crt ]]; then
	  ...
	  vvvsigncert="$(openssl req -new -x509 \
            ...
            -days 3650 \
            -subj /CN=*.wordpress-develop.php53/CN=*.wordpress.php53/CN=*.local.php53/CN=*.wordpress-trunk.php53 2>&1)"
	  echo "$vvvsigncert"
  fi
...
PHP
    ...
    noroot wp core install --url=local.wordpress.php53 --quiet --title="Local WordPress Dev" --admin_name=admin --admin_email="admin@local.dev" --admin_password="password"
  else
...
PHP
    ...
    noroot wp core install --url=local.wordpress-trunk.php53 --quiet --title="Local WordPress Trunk Dev" --admin_name=admin --admin_email="admin@local.dev" --admin_password="password"
  else
...
// Allow (src|build).wordpress-develop.php53 to share the same Database
define( 'WP_HOME', 'http://build.wordpress-develop.php53' );
define( 'WP_SITEURL', 'http://build.wordpress-develop.php53' );
}

define( 'WP_DEBUG', true );
PHP
    echo "Installing WordPress develop..."
    noroot wp core install --url=src.wordpress-develop.php53 --quiet --title="WordPress Develop" --admin_name=admin --admin_email="admin@local.dev" --admin_password="password"
...
echo "For further setup instructions, visit http://local.php53"
